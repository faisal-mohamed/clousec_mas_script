...
async function createInstanceWithoutProfile(vpcId) {
    try {
        const timestamp = Date.now();
        const subnetId = await getSubnetFromVpc(vpcId);

        // Create instance without instance profile
        const createParams = {
            ...
            // No IamInstanceProfile parameter here
            ...
            TagSpecifications: [
                {
                    ...
                    Tags: [
                        {
                            Key: 'Name',
                            Value: `no-profile-instance-${timestamp}`
                        },
                        {
                            Key: 'simulation-mas',
                            Value: 'true'
                ...
                },
                {
                    ...
                    Tags: [
                        {
                            Key: 'Name',
                            Value: `no-profile-volume-${timestamp}`
                        },
                        {
                            Key: 'simulation-mas',
                            Value: 'true'
        ...
        return {
            ...
            iamInstanceProfile: instance.IamInstanceProfile || 'None'
        };
...
async function main() {
    try {
        ...
        const result = await createInstanceWithoutProfile(process.env.VPC_ID);
        ...
        console.log('\nIAM Configuration:');
        console.log('- No instance profile attached');
        console.log('- No IAM role available');
        console.log('- No AWS service access');
        console.log('- No AWS API permissions');
        console.log('- Limited functionality');

        console.log('\nNon-compliant settings:');
        console.log('1. No instance profile configured');
        console.log('2. Cannot access AWS services');
        console.log('3. No AWS API permissions');
        console.log('4. Limited instance capabilities');
        console.log('5. Manual credential management required');

        console.log('\nSecurity Implications:');
        console.log('1. No managed AWS credentials');
        console.log('2. Potential insecure credential storage');
        console.log('3. No automated credential rotation');
        console.log('4. Limited AWS service integration');
