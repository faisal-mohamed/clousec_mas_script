...
async function getAvailableSubnets(vpcId) {
    try {
        ...
        // Select two subnets from different AZs for the subnet group requirement
        ...
        return {
            allSubnetIds: selectedSubnets.map(subnet => subnet.SubnetId),
            primaryAZ: selectedSubnets[0].AvailabilityZone
        };
    } catch (error) {
...
async function createSingleAZRDSInstance(vpcId) {
    try {
        const { allSubnetIds, primaryAZ } = await getAvailableSubnets(vpcId);
        const timestamp = Date.now().toString().slice(-4);
        const instanceIdentifier = `rds-single-az-${timestamp}`;

        console.log('Creating RDS instance in single AZ...');
        const params = {
           ...
           // EngineVersion: '8.0.28',
            ...
            MultiAZ: false,
            AvailabilityZone: primaryAZ,  // Explicitly specify the AZ
            DBSubnetGroupName: await createDBSubnetGroup(instanceIdentifier, allSubnetIds),
            Tags: [
                ...commonTags,
                {
...
createSingleAZRDSInstance(vpcId)
    .then(instance => {
        ...
        console.log('\nHigh Availability Configuration:', {
            ...
            DeploymentAZ: instance.AvailabilityZone,
            AutomaticFailover: 'Not Available',
