...
async function createDBSubnetGroup(identifier, subnetIds) {
    try {
        ...
        await rdsClient.send(new CreateDBSubnetGroupCommand({
            ...
            SubnetIds: subnetIds,
            Tags: commonTags  // Add simulation-mas tag
        }));
...
async function createRDSWithoutDeletionProtection(vpcId) {
    try {
        const subnetIds = await getAvailableSubnets(vpcId);
        const timestamp = Date.now().toString().slice(-4);
        const instanceIdentifier = `rds-no-protection-${timestamp}`;
        ...
        const params = {
            ...
            Tags: [
                ...commonTags,                  // Add simulation-mas tag
                {
            ...
            MonitoringInterval: 0,              // Basic monitoring only
            EnablePerformanceInsights: false    // No performance insights
        };
...
// Execute the creation with VPC ID from environment
createRDSWithoutDeletionProtection(vpcId)
    .then(instance => {
