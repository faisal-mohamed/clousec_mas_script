...
async function createMinimalCostEFS() {
    try {
        const params = {
            PerformanceMode: 'generalPurpose', // Cheaper than 'maxIO'
            ThroughputMode: 'bursting',        // More cost-effective than 'provisioned'
            Encrypted: true,                   // Keeping encryption for security
            Backup: false,                     // Disabling backups to reduce costs
            Tags: [
                {
                    Key: 'simulation-mas',
                    Value: 'true'
                ...
                {
                    Key: 'Name',
                    Value: `minimal-cost-efs-${Date.now()}`
                },
                {
                    Key: 'CreatedBy',
                    Value: 'automation'
        ...
        console.log('Low-cost File System created:', {
...
async function main() {
    try {
        ...
        // Create the minimal cost file system
        await createMinimalCostEFS();
