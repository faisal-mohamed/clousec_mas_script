...
async function createNonCompliantCloudTrail() {
  ...
  try {
    ...
    // Create CloudTrail trail without encryption
    ...
    const createTrailResponse = await cloudTrailClient.send(new CreateTrailCommand({
      ...
      // Intentionally omitting KmsKeyId to make it non-compliant (unencrypted)
      Tags: [
        {
          Key: 'simulation-mas',
          Value: 'true'
    ...
    console.log('Encryption: Disabled (non-compliant)');
