...
// Create RDS instance with auto minor version upgrade disabled (non-compliant)
const createNonCompliantRDSInstance = async (subnetId) => {
    ...
    try {
        const params = {
            ...
            AutoMinorVersionUpgrade: false, // Non-compliant setting
            BackupRetentionPeriod: 0, // Disable automated backups for faster deletion
            DeletionProtection: false
        };

        console.log('Creating RDS instance with auto minor version upgrade disabled...');
...
// Make RDS instance compliant (for demonstration)
const makeCompliant = async (instanceIdentifier) => {
    const rdsClient = getClient(RDSClient);

    try {
        console.log('Enabling auto minor version upgrade...');
        ...
        await waitForInstanceStatus(rdsClient, instanceIdentifier, 'available');
        console.log('Auto minor version upgrade enabled successfully');
    } catch (error) {
...
const main = async () => {
    ...
    try {
        ...
        const { subnetId } = await getDefaultVpcAndSubnet();

        // Create non-compliant RDS instance
        instanceIdentifier = await createNonCompliantRDSInstance(subnetId);

        // Wait to observe the non-compliant state
        console.log('\nWaiting 60 seconds to observe non-compliant state...');
        await new Promise(resolve => setTimeout(resolve, 60000));

        // Optional: Make the instance compliant (uncomment if needed)
